...
def get_dataloader(conf):
    ...
    if conf.dataset == 'tooth':
        ...
        train_loader = DataLoader(trainset, batch_size=conf.batch_size, shuffle=True, num_workers=conf.num_workers, collate_fn=collate_fn)
        valset = OMNIDataset(os.path.join(root, "val"), os.path.join(annFile, 'instances_val.json'), train=False)
        val_loader = DataLoader(valset, batch_size=conf.batch_size, shuffle=False, num_workers=conf.num_workers, collate_fn=collate_fn)
