...
def load_contrastive_weights(model, checkpoint_path, device):
    """
    Load contrastive weights with proper architecture matching
    """
    if not os.path.exists(checkpoint_path):
        print(f"Contrastive backbone not found at {checkpoint_path}, training from scratch.")
        return

    try:
        # Load the contrastive checkpoint
        contrastive_state = torch.load(checkpoint_path, map_location=device)

        # Get the FPN backbone state dict
        fpn_backbone_state = model.faster_rcnn.backbone.body.state_dict()

        # Create mapping for compatible layers
        compatible_dict = {}

        for fpn_key, fpn_param in fpn_backbone_state.items():
            # The FPN backbone might have the same layer names as plain ResNet
            if fpn_key in contrastive_state:
                if contrastive_state[fpn_key].shape == fpn_param.shape:
                    compatible_dict[fpn_key] = contrastive_state[fpn_key]
                else:
                    print(f"Shape mismatch for {fpn_key}: "
                          f"checkpoint {contrastive_state[fpn_key].shape} vs "
                          f"model {fpn_param.shape}")

        if len(compatible_dict) > 0:
            # Update the FPN backbone with compatible weights
            fpn_backbone_state.update(compatible_dict)
            model.faster_rcnn.backbone.body.load_state_dict(fpn_backbone_state)
            print(f"Successfully loaded {len(compatible_dict)} layers from contrastive checkpoint")
        else:
            print("No compatible layers found - training from scratch")

    except Exception as e:
        print(f"Error loading contrastive weights: {e}")
...
def main(conf):
    ...
    print(f"Using architecture: {conf.arc}")
